...
pub mod PT {
    ...
    // Refinement of l1
            ...
            result_map_ok(res, |v: (usize, PageTableEntryExec)| (v.0 as nat, v.1@)) === interp_at(
                mem,
                pt,
                layer as nat,
                ptr,
...

    {
        ...
        if entry.is_mapping() {
            ...
            if entry.is_dir(layer) {
                ...
                assert(result_map_ok(res, |v: (usize, PageTableEntryExec)| (v.0 as nat, v.1@))
                ...
                assert(result_map_ok(res, |v: (usize, PageTableEntryExec)| (v.0 as nat, v.1@).0)
                    === result_map_ok(
                    interp@.resolve(vaddr as nat),
                    |v: (nat, PageTableEntry)| v.0,
                ));
                assert(result_map_ok(
                    res,
            ...
            assert(result_map_ok(Err(()), |v: (usize, PageTableEntryExec)| (v.0 as nat, v.1@))
                === interp@.resolve(vaddr as nat));
            Err(())
        }
    }
    ...
    // Refinement of l1
            ...
            result_map_ok(res, |v: (usize, PageTableEntryExec)| (v.0 as nat, v.1@)) === interp(
                mem,
                pt,
            ).resolve(vaddr as nat),
            // Refinement of l0
...

    {
        ...
        if entry.is_mapping() {
            ...
            if x86_arch_exec().entry_size(layer) == pte.frame.size {
                ...
                let (pt_with_empty, new_dir_region, new_dir_entry) = insert_empty_directory(
                    mem,
                    Ghost(pt),
                    layer,
                    ptr,
    ...
    pub fn map_frame(
        ...
        ensures
            ...
            // Refinement of l0
            match res {
                Ok(_) => Ok(interp(mem, pt@).interp()) === interp(
                    &*old(mem),
                    old(pt)@,
                ).interp().map_frame(vaddr as nat, pte@),
                Err(_) => Err(interp(mem, pt@).interp()) === interp(
                    &*old(mem),
                    old(pt)@,
                ).interp().map_frame(vaddr as nat, pte@),
            },
    ...
    fn insert_empty_directory(
        ...
        ,
        PageDirectoryEntry  /* new_dir_entry */
        ,
    ))
        requires
            inv_at(&*old(mem), pt, layer as nat, ptr),
        ...
        ensures
            ...
            forall|i: nat|
                i < 512 && i != idx ==> view_at(mem, res.0@, layer as nat, ptr, i) == view_at(
                    &*old(mem),
                    res.0@,
                    layer as nat,
                    ptr,
            ...
            forall|r: MemRegion|
                r != res.0@.region && r != res.0@.entries[idx as int].get_Some_0().region
                    ==> mem.region_view(r) == old(mem).region_view(r),
            ({
                let pt_res = res.0@;
                let new_dir_region = res.1;
                let new_dir_entry = res.2;
                ...
                let new_dir_interp = interp_at(
                    mem,
                    pt_res.entries[idx as int].get_Some_0(),
                    (layer + 1) as nat,
                    new_dir_region.base,
                    entry_base,
                );
                ...
                &&& view_at(mem, pt_res, layer as nat, ptr, idx as nat).get_Directory_addr()
                    == new_dir_region.base
            ...
            }),
    {
        ...
        proof {
            ...
            assert(forall|r: MemRegion|
                r !== new_dir_pt@.region && r !== pt_res@.region ==> mem_with_empty@.region_view(r)
                    === old(mem).region_view(r));
            assert(mem_with_empty@.region_view(pt_res@.region) === old(mem).region_view(
                pt_res@.region,
            ).update(idx as int, new_dir_entry.entry));
...

    {
        ...
        assert(interp_at_entry(mem, pt, layer as nat, ptr, base as nat, idx as nat) == interp_at(
            mem,
            pt,
            layer as nat,
            ptr,
                            ...
                            proof {
                                ...
                                assert(Ok(interp_at(mem, pt_res@, layer as nat, ptr, base as nat))
                                    ...
                                    assert(interp_at(
                                        ...
                                        ptr,
                                        base as nat,
                                    ).unmap(vaddr as nat).get_Ok_0().entries);
                                };
                            }
                            Ok(res)
                        } else {
                ...
                if aligned_exec(vaddr, x86_arch_exec().entry_size(layer)) {
                    ...
                    proof {
                        ...
                        assert(Ok(interp_at(mem, pt, layer as nat, ptr, base as nat)) === interp_at(
                            ...
                            assert(interp_at(mem, pt, layer as nat, ptr, base as nat).entries
                                =~= interp_at(&*old(mem), pt, layer as nat, ptr, base as nat).unmap(
                                vaddr as nat,
                            ).get_Ok_0().entries);
                        };
