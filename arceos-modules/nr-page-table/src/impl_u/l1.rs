...
impl Directory {
        ...
        decreases self.arch.layers.len() - self.layer,
    {
        ...
        assert(aligned(self.base_vaddr, self.entry_size())) by {
            extra::mod_mult_zero_implies_mod_zero(
                self.base_vaddr,
                self.entry_size(),
                self.num_entries(),
            );
        ...
        match self.entries.index(entry as int) {
            ...
            NodeEntry::Empty() => {
                if self.entry_size() == pte.frame.size {
                    ...
                    assert(new_dir.interp().map_frame(base, pte).get_Ok_0().map
                        =~= map![base => pte]);
        ...
        decreases self.arch.layers.len() - self.layer, self.num_entries() - i,
    {
        if i >= self.entries.len() {
            ...
            assert(rem1.map.union_prefer_right(entry_i1.map) =~= rem2.map.union_prefer_right(
                entry_i2.map,
            ));
        }
    }
